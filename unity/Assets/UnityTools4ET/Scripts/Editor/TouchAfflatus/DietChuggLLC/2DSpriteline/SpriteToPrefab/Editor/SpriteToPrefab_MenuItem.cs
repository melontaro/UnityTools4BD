...
public class SpriteToPrefab_MenuItem
{
    ...
    public static void ScriptableObjectTemplateMenuItem()
    {
        ...
        for (int i = 0; i < Selection.objects.Length; i++)
        {
            ...
            string fullFolderPathShort = Inset(spriteSheet, 0, splitSpriteSheet[splitSpriteSheet.Length - 1].Length + 1);
            ...
            if (!AssetDatabase.IsValidFolder(fullFolderPath) && makeSeperateFolders)
            {
            ...
            for (int j = 0; j < sprites.Length; j++)
            {
                ...
                string tempFolderPath = makeSeperateFolders ? fullFolderPath : fullFolderPathShort;
                string savePath = makeSeperateFolders ? tempFolderPath + "/" + sprites[j].name + "/" + sprites[j].name + ".prefab" : tempFolderPath + "/" + sprites[j].name + ".prefab";
